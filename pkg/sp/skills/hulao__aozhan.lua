local hulao__aozhan = fk.CreateSkill {
  name = "hulao__aozhan"
}
...
hulao__aozhan:addEffect(fk.DrawNCards, {
  anim_type = "offensive",
  frequency = Skill.Compulsory,
  can_trigger = function(skill, event, target, player)
  if not player:hasSkill(skill.name) or target ~= player then return false end
...
hulao__aozhan:addEffect(fk.DamageInflicted, {
  anim_type = "offensive",
  frequency = Skill.Compulsory,
...
hulao__aozhan:addEffect(fk.EventPhaseChanging, {
  anim_type = "offensive",
  frequency = Skill.Compulsory,
  can_trigger = function(skill, event, target, player, data)
  if not player:hasSkill(skill.name) or target ~= player then return false end
...
hulao__aozhan:addEffect('targetmod', {
  anim_type = "offensive",
  residue_func = function(skill, player, skill_name, scope)
  if player:hasSkill(hulao__aozhan.name) and #player:getEquipments(Card.SubtypeWeapon) > 0 and
