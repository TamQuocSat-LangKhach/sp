  ...
  on_use = function(self, event, target, player, data)
    ...
    if event == fk.DrawNCards then
      ...
      if #player.player_cards[Player.Hand] + #player.player_cards[Player.Equip] <= #kingdoms then
        ...
        player.room:askForDiscard(player, #kingdoms, #kingdoms, true, self.name, false, ".", "#yongsi-discard")
      end
    end
  end,
}
...
Fk:loadTranslationTable{
  ...
  ["#yongsi-discard"] = "庸肆：你至少须弃掉等同于场上现存势力数的牌",
}
...
local juesi = fk.CreateActiveSkill{
  ...
  on_use = function(self, room, effect)
    ...
    local card = Fk:getCardById(room:askForDiscard(target, 1, 1, true, self.name, false, ".", "#juesi-discard")[1])
...
Fk:loadTranslationTable{
  ...
  ["#juesi-discard"] = "决死：你需弃置一张牌，若不为【杀】且你体力值不小于其，视为其对你使用【决斗】",
}

--local sp__caiwenji = General(extension, "sp__caiwenji", "wei", 3, 3, General.Female)
local chenqing = fk.CreateTriggerSkill{
  ...
  on_cost = function(self, event, target, player, data)
    ...
    local to = room:askForChoosePlayers(player, targets, 1, 1, "#chenqing-choose", self.name)
    if #to > 0 then
      self.cost_data = to[1]
      return true
    end
...
local mozhi = fk.CreateTriggerSkill{
  ...
  can_trigger = function(self, event, target, player, data)
    if target == player and player:hasSkill(self.name) and player.phase == Player.Finish and not player:isKongcheng() then
      return player.tag[self.name] and #player.tag[self.name] > 0
    end
  end,
  on_cost = function(self, event, target, player, data)
    self.cost_data = player.room:askForUseCard(player, player.tag[self.name][1], ".", "#mozhi_use", true)
  ...
  can_refresh = function(self, event, target, player, data)
    if target == player and player:hasSkill(self.name, true) and player.phase ~= Player.NotActive then
      if event == fk.CardUseFinished then
      return data.card.type == Card.TypeBasic or (data.card.type == Card.TypeTrick and data.card.sub_type ~= Card.SubtypeDelayedTrick)
      else
...
--sp__caiwenji:addSkill(chenqing)
--sp__caiwenji:addSkill(mozhi)
Fk:loadTranslationTable{
  ["sp__caiwenji"] = "蔡文姬",
  ["chenqing"] = "陈情",
  ...
  ["#mozhi-use"] = "默识：你可以将一张手牌当【珠泪哀歌族·水仙女人鱼】使用",
}
...
local yuanhu_active = fk.CreateActiveSkill{
  ...
  on_use = function(self, room, effect)
    ...
    if card.sub_type == Card.SubtypeWeapon then
      ...
      local to = room:askForChoosePlayers(player, targets, 1, 1, "#yuanhu-choose", self.name, false)
      if #to > 0 then
...
Fk:loadTranslationTable{
  ...
  ["#yuanhu-choose"] = "援护：弃置该角色距离1的一名角色区域中的一张牌",
}
...
local xueji = fk.CreateActiveSkill{
  ...
  can_use = function(self, player)
    return player:isWounded() and player:usedSkillTimes(self.name) == 0 and not player:isKongcheng()
  end,
  card_filter = function(self, to_select, selected, targets)
...
local wuji = fk.CreateTriggerSkill{
  ...
  on_use = function(self, event, target, player, data)
    player.room:handleAddLoseSkills(player, "-huxiao", nil)
  end,

  refresh_events = {fk.Damage, fk.EventPhaseStart},
  can_refresh = function(self, event, target, player, data)
    if target == player and player:hasSkill(self.name, true) and player:getMark(self.name) == 0 then
  ...
  on_refresh = function(self, event, target, player, data)
    local room = player.room
    if event == fk.Damage then
      room:addPlayerMark(player, "wuji_record", data.damage)
    else
      room:addPlayerMark(player, "wuji_record", 0)
    end
  end,
}
...
local tianming = fk.CreateTriggerSkill{
  ...
  on_cost = function(self, event, target, player, data)
    if player:isNude() then
      ...
      local cards = player.room:askForDiscard(player, math.min(2, #player.player_cards[Player.Hand] + #player.player_cards[Player.Equip]), 2, true, self.name, true, ".", "#tianming-cost")
      if #cards >= math.min(2, #player.player_cards[Player.Hand] + #player.player_cards[Player.Equip]) then
        return true
      end
    end
  end,
  on_use = function(self, event, target, player, data)
    ...
    if to:isNude() then
      ...
      local cards = room:askForDiscard(to, math.min(2, #to.player_cards[Player.Hand] + #to.player_cards[Player.Equip]), 2, true, self.name, true, ".", "#tianming-cost")
      if #cards >= math.min(2, #to.player_cards[Player.Hand] + #to.player_cards[Player.Equip]) then
        to:drawCards(2)
      end
    end
  end,
...
local jieyuan = fk.CreateTriggerSkill{
  ...
  on_cost = function(self, event, target, player, data)
    ...
    return #player.room:askForDiscard(player, 1, 1, false, self.name, true, pattern, "#jieyuan-cost") > 0
  end,
  on_use = function(self, event, target, player, data)
    if event == fk.DamageCaused then
...
Fk:loadTranslationTable{
  ...
  ["#jieyuan-cost"] = "竭缘：弃置一张黑色手牌令此伤害+1/红色手牌令此伤害-1",
}
...
local bifa = fk.CreateTriggerSkill{
  ...
  on_cost = function(self, event, target, player, data)
    local tos, id = player.room:askForChooseCardAndPlayers(player, self.bifa_tos, 1, 1, ".|.|.|hand|.|.", "#bifa-cost", self.name)
    if #tos > 0 then
  ...
  on_refresh = function(self, event, target, player, data)
    ...
    local card = room:askForCard(target, 1, 1, false, self.name, true, ".|.|.|hand|.|"..type, "#bifa-invoke")
    if #card > 0 then
...
Fk:loadTranslationTable{
  ...
  ["#bifa-invoke"] = "笔伐：交出一张同类型的手牌并获得此牌；或点“取消”将此牌置入弃牌堆并失去1点体力",
}
...
local xingwu = fk.CreateTriggerSkill{
  ...
  on_cost = function(self, event, target, player, data)
    local colors = player:getMark(self.name)
  ...
  can_refresh = function(self, event, target, player, data)
    if player:hasSkill(self.name, true) and target == player then
  ...
  on_refresh = function(self, event, target, player, data)
    ...
    if event == fk.CardUsing then
      ...
      room:setPlayerMark(player, self.name, colors)
    else
...
local yanyu_give = fk.CreateTriggerSkill{
  ...
  on_cost = function(self, event, target, player, data)
    local to = player.room:askForChoosePlayers(player, table.map(player.room:getAlivePlayers(), function(p) return p.id end), 1, 1, "#yanyu-choose", self.name)
    if #to > 0 then
      self.cost_data = to[1]
      return true
    end
...
local xiaoguo = fk.CreateTriggerSkill{
  ...
  on_cost = function(self, event, target, player, data)
    return #player.room:askForDiscard(player, 1, 1, false, self.name, true, ".|.|.|.|.|basic", "#xiaoguo-cost") > 0
  end,
  on_use = function(self, event, target, player, data)
    local room = player.room
    if #room:askForDiscard(target, 1, 1, true, self.name, true, ".|.|.|.|.|equip", "#xiaoguo-discard") > 0 then
      player:drawCards(1)
    else
      room:damage{
        from = player,
...
Fk:loadTranslationTable{
  ...
  ["#xiaoguo-cost"] = "骁果：你可以弃置一张基本牌，该角色需弃置一张装备牌并令你摸一张牌，否则你对其造成1点伤害",
  ["#xiaoguo-discard"] = "骁果：你需弃置一张装备牌并令其摸一张牌，否则其对其造成1点伤害",
}
...
local zhoufu = fk.CreateActiveSkill{
  ...
  target_filter = function(self, to_select, selected, cards)
    return #selected == 0 and to_select ~= Self.id and #Fk:currentRoom():getPlayerById(to_select):getPile("zhou") == 0
  end,
  on_use = function(self, room, effect)
    local target = room:getPlayerById(effect.tos[1])
    target:addToPile("zhou", effect.cards, false, self.name)
  end,
}
local zhoufu_trigger = fk.CreateTriggerSkill{
  ...
  events = {fk.EventPhaseStart},
  can_trigger = function(self, event, target, player, data)
    return #target:getPile("zhou") > 0 and player:hasSkill(self.name) and target.phase == Player.NotActive
  end,
  on_cost = function(self, event, target, player, data)
    return true
  end,
  on_use = function(self, event, target, player, data)
    target:removeCards(Player.Special, target:getPile("zhou"), "zhoufu")
    player.room:moveCards({
      from = target.id,
      ids = target:getPile("zhou"),
      to = player.id,
  ...
  can_refresh = function(self, event, target, player, data)
    return #target:getPile("zhou") > 0
  end,
  on_refresh = function(self, event, target, player, data)
    data.card = Fk:getCardById(target:getPile("zhou")[1])
...
local kangkai = fk.CreateTriggerSkill{
  ...
  on_use = function(self, event, target, player, data)
    ...
    local cards = room:askForCard(player, 1, 1, true, self.name, false, ".", "#kangkai-give")
    if #cards > 0 then
...
local hongyuan = fk.CreateTriggerSkill{
  ...
  on_cost = function(self, event, target, player, data)
    local tos = player.room:askForChoosePlayers(player, table.map(player.room:getOtherPlayers(player), function(p) return p.id end), 1, 2, "#hongyuan-cost", self.name)
    if #tos > 0 then
      self.cost_data = tos
      return true
    end
...
local qiangwu = fk.CreateActiveSkill{
  ...
  on_use = function(self, room, effect)
    ...
    room:judge(judge)
    room:setPlayerMark(player, "@qiangwu", judge.card.number)
  end,
}
local qiangwu_record = fk.CreateTriggerSkill{
  ...
  refresh_events = {fk.AfterCardUseDeclared, fk.EventPhaseStart},
  can_refresh = function(self, event, target, player, data)
    if target == player and player:hasSkill(self.name) and player:getMark("@qiangwu") > 0 then
      if event == fk.AfterCardUseDeclared then
        return data.card.trueName == "slash" and data.card.number and data.card.number ~= player:getMark("@qiangwu")
      else
        return player.phase == Player.NotActive
      end
    end
  ...
  on_refresh = function(self, event, target, player, data)
    local room = player.room
    if event == fk.AfterCardUseDeclared then
      if data.card.number > player:getMark("@qiangwu") then
        player:addCardUseHistory(data.card.trueName, -1)
      end
      --TODO: no distance limit
    else
      player.room:setPlayerMark(player, "@qiangwu", 0)
    end
  end,
}
...
local duanbing = fk.CreateTriggerSkill{
  ...
  on_cost = function(self, event, target, player, data)
    ...
    local to = room:askForChoosePlayers(player, targets, 1, 1, "#duanbing-choose", self.name)
    if #to > 0 then
      self.cost_data = to[1]
      return true
    end
...
local zhendu = fk.CreateTriggerSkill{
  ...
  on_cost = function(self, event, target, player, data)
    return #player.room:askForDiscard(player, 1, 1, false, self.name, true, ".|.|.|hand|.|.", "#zhendu-cost") > 0
  end,
  on_use = function(self, event, target, player, data)
    local room = player.room
...
Fk:loadTranslationTable{
  ...
  ["#zhendu-cost"] = "鸩毒：你可以弃置一张手牌视为该角色使用一张【酒】，然后你对其造成1点伤害",
}
...
local mumu = fk.CreateTriggerSkill{
  ...
  on_cost = function(self, event, target, player, data)
    local to = player.room:askForChoosePlayers(player, self.mumu_tos, 1, 1, "#mumu-choose", self.name)
    if #to > 0 then
      self.cost_data = to[1]
      return true
    end
...
local zhenwei = fk.CreateTriggerSkill{
  ...
  can_trigger = function(self, event, target, player, data)
    return player:hasSkill(self.name) and data.firstTarget and data.from ~= player.id and
  ...
  on_cost = function(self, event, target, player, data)
    return #player.room:askForDiscard(player, 1, 1, true, self.name, true, ".", "#zhenwei-cost") > 0
  end,
  on_use = function(self, event, target, player, data)
    local room = player.room
    ...
    if choice == "zhenwei_transfer" then
      player:drawCards(1, self.name)
      --FIXME: target filter, collateral
  ...
  refresh_events = {fk.EventPhaseStart},
  can_refresh = function(self, event, target, player, data)
    return target.phase >= Player.NotActive and #player:getPile(self.name) > 0
  end,
  on_refresh = function(self, event, target, player, data)
    local dummy = Fk:cloneCard("dilu")
    dummy:addSubcards(player:getPile(self.name))
    player.room:obtainCard(player.id, dummy, true)
  end,
}
...
Fk:loadTranslationTable{
  ...
  ["#zhenwei-cost"] = "镇卫：你可以弃置一张牌，将此牌转移给你，或取消之并令使用者回合结束时收回",
...
local junbing = fk.CreateTriggerSkill{
  ...
  on_use = function(self, event, target, player, data)
    ...
    local cards = room:askForCard(player, #dummy1.subcards, #dummy1.subcards, false, self.name, false, ".", "#junbing-give")
