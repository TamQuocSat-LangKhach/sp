...
local xueji = fk.CreateActiveSkill{
  ...
  can_use = function(self, player)
    return player:isWounded() and player:usedSkillTimes(self.name, Player.HistoryTurn) == 0 and not player:isNude()
  end,
  card_filter = function(self, to_select, selected, targets)
...
local sp__yanyu = fk.CreateTriggerSkill{
  ...
  on_use = function(self, event, target, player, data)
    ...
    if not player.dead then
      ...
      room:setPlayerMark(player, "@yanyu-phase", type)
    end
  end,
}
local yanyu_give = fk.CreateTriggerSkill{
  ...
  can_trigger = function(self, event, target, player, data)
    if player:hasSkill("sp__yanyu") and player:getMark("@yanyu-phase") ~= 0 and player:usedSkillTimes(self.name, Player.HistoryTurn) < 3 then
      local room = player.room
  ...
  on_trigger = function(self, event, target, player, data)
    ...
    if #ids > 0 then
      for _, id in ipairs(ids) do
        if player:usedSkillTimes(self.name, Player.HistoryTurn) < 3 and not player.dead and player.room:getCardArea(id) == Card.DiscardPile then
          self:doCost(event, nil, player, id)
        end
      end
    end
  end,
  on_cost = function(self, event, target, player, data)
    local to = player.room:askForChoosePlayers(player, table.map(player.room:getAlivePlayers(), function(p)
      return p.id end), 1, 1, "#yanyu-choose:::"..Fk:getCardById(data):toLogString(), "sp__yanyu", true)
    if #to > 0 then
      self.cost_data = {to[1], data}
      return true
    end
  end,
  on_use = function(self, event, target, player, data)
...
Fk:loadTranslationTable{
  ...
  ["#xiaode-choice"] = "孝德：选择要获得 %src 的技能",
...
local hongyuan = fk.CreateTriggerSkill{
  ...
  on_cost = function(self, event, target, player, data)
    local tos = player.room:askForChoosePlayers(player, table.map(player.room:getOtherPlayers(player),
      function(p) return p.id end), 1, 2, "#hongyuan-cost", self.name, true)
    if #tos > 0 then
      self.cost_data = tos
      return true
    end
...
local zishu = fk.CreateTriggerSkill{
  ...
  can_trigger = function(self, event, target, player, data)
    if player:hasSkill(self.name) then
      if event == fk.TurnEnd and target ~= player then
        local cards = player:getMark("zishu-turn")
        return cards ~= 0 and #cards > 0
      else
  ...
  on_refresh = function(self, event, target, player, data)
    ...
    player.room:setPlayerMark(player, "zishu-turn", cards)
  end,
}
